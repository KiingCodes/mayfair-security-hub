...
import {
  Upload, FileText, Trash2, Download, Loader2, File, Search,
...
const AdminFiles = () => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !user || !selectedClient) {
      if (!selectedClient) toast({ title: "Select a client first", variant: "destructive" });
      return;
    }
          ...
          <div className="relative">
            <Button disabled={uploading || !selectedClient || selectedClient === "all"} className="relative">
              ...
              <input
                ...
                disabled={uploading || !selectedClient || selectedClient === "all"}
              />
            </Button>
          </div>
        </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                    <span className="flex items-center gap-1">
                      <User className="w-3 h-3" /> {getClientLabel(file.client_id)}
                    </span>
                    <span>•</span>
                    ...
                    <span>•</span>
                    <span className={isAdmin ? "text-primary" : "text-accent"}>
